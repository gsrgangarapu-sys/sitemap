...
function normalizeUrl(url: string | undefined): string {
  if (!url) return '';
  // Replace double slashes but preserve protocol '://'
...
export function generateSitemapIndexXml(sitemaps: { loc: string, lastmod?: string }[]): string {
  const sitemapsContent = sitemaps
    .map(
      (s) => `
  <sitemap>
    <loc>${escapeXml(normalizeUrl(s.loc))}</loc>
...
<head>
    ...
    <title>Sitemap for ${escapeXml(domain)}</title>
    <style>
...
<body>
    <h1>Sitemap for ${escapeXml(domain)}</h1>
